...
Follow any deeper `AGENTS.md` files for package-local details. The root guide and root README are the source of truth for product shape, app-domain ownership, and public repository posture.
...
Devys is a native macOS, terminal-first AI development environment.

The target product shape is:

- native macOS terminal windows and tabs as the primary layout model
- optional project binding per native window group
- transient or pinned left project drawer for files and changes
- transient or pinned bottom composer drawer
- fast read-only file and diff tabs using shared editor/syntax capabilities
- app-local project and git/status policy until those domains justify reusable packages

The app should feel explicit, warm, native, and fast. Power users should be able to stay keyboard-first. New contributors should be able to identify state ownership, effect ownership, and UI ownership immediately.

## Public Docs

Keep the root `README.md` accurate for GitHub readers. It should make the active product, build path, permission posture, and distribution strategy obvious without exposing local planning notes, generated dependency builds, archived experiments, or machine-local automation state.
...
- `Apps/mac-client`
  Native macOS terminal-first product host. It owns native windows, tabs, product chrome, project drawer policy, file/diff tab presentation policy, and app-local project binding. It consumes `Packages/Terminal`, `Packages/Editor`, and shared UI/capability packages; it must not fork terminal runtime ownership from `Packages/Terminal`.
- `Packages/UI`
  The only design-system source of truth. Tokens, shared styling, and stateless reusable components live here.
- `Packages/Editor`, `Packages/Syntax`, `Packages/Text`, `Packages/Rendering`, `Packages/SSH`, `Packages/RemoteCore`
  Capability and engine packages. Keep their boundaries narrow. They are not app-domain owners.
- `Packages/Terminal`
  Reusable terminal package for the Devys Terminal product. `TerminalVT`, `TerminalHost`, and `TerminalComposer` expose engine/composer capabilities; `TerminalProduct` stays a thin product composition layer and must not become app-domain authority.
Do not route new production code through archived or ignored material.
...
- Interaction policy belongs in the active app shell or explicit feature logic, not in `Packages/UI`.
...
- **Three surfaces.** `base` (terminal/window background), `material` (transient or pinned drawers), `overlay` (modals/popovers). Apply through shared UI primitives rather than feature-local styling.
...
- terminal base as the primary canvas
- left project drawer, transient or pinned
- bottom composer drawer, transient or pinned
- native AppKit windows and tab groups for terminal, file, and diff tabs
- overlay surfaces for low-frequency commands

Do not reintroduce the old monolith repo rail, split-pane card grid, or four-mode sidebar framing.
...
- If you touch migration status or the remaining ownership boundary, update root-visible guidance when the public product shape changes.
