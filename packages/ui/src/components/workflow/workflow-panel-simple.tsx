...
interface WorkflowPanelSimpleProps {
  className?: string;
  activeChatSessionId?: string | null;
}

export function WorkflowPanelSimple({ className, activeChatSessionId }: WorkflowPanelSimpleProps) {
  ...
  useEffect(() => {
    ...
    // This will be handled by the WebSocketProvider's onMessage prop
    // For now, we'll need to pass this handler up to the provider
  }, []);
      ...
      <Card className="m-4">
        <CardHeader>
          ...
          <CardDescription>
            ...
            {!isConnected && <span className="text-yellow-600 ml-2">(WebSocket disconnected)</span>}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!execution && (
            <div className="space-y-4">
              ...
              <Button 
                ...
                disabled={isStarting || !activeChatSessionId || !isConnected}
                className="w-full"
              >
                {isStarting ? (
                  <>
