...
final class DiffDisplayModel {
    ...
    private func makeUnifiedRow(
        ...
        return VisibleUnifiedDiffDisplayRow(
            ...
            oldLineNumberPacket: row.oldLineNumber.map {
                makePlainPacket(
                    text: String($0),
                    foregroundColor: lineNumberColor(for: row.lineType, theme: diffTheme),
    ...
    private func splitSide(_ request: DiffSplitSideRequest) -> SplitDiffDisplaySide {
        ...
        return SplitDiffDisplaySide(
            ...
            lineNumberPacket: request.side.lineNumber.map {
                makePlainPacket(
                    text: String($0),
                    foregroundColor: lineNumberColor(for: request.side.lineType, theme: diffTheme),
    ...
    private func textForToken(_ token: SyntaxHighlightToken, in text: String) -> String {
        ...
        let startIdx = text.diffUTF16Index(at: tokenStart)
        let endIdx = text.diffUTF16Index(at: tokenEnd)
...
private extension String {
    func diffUTF16Index(at offset: Int) -> String.Index {
        guard offset > 0 else { return startIndex }
        guard offset < utf16.count else { return endIndex }
