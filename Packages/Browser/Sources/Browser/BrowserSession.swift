...
public final class BrowserSession: Identifiable {
    ...
    /// Root directory local file loads may read for relative assets.
    private var fileReadAccessURL: URL?
    ...
    public init(id: UUID = UUID(), url: URL, fileReadAccessURL: URL? = nil) {
        self.id = id
        self.url = url
    ...
    public func loadFile(url: URL, allowingReadAccessTo readAccessURL: URL) {
        self.url = url.standardizedFileURL
        fileReadAccessURL = readAccessURL.standardizedFileURL
        clearError()
        if let webView {
            load(self.url, in: webView)
        }
    }
    ...
    private func load(_ url: URL, in webView: WKWebView) {
        if url.isFileURL {
            let readAccessURL = fileReadAccessURL ?? url.deletingLastPathComponent()
