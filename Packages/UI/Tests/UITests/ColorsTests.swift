...
struct ColorsTests {
    ...
    @Test("Theme registry exposes light, dark, and system modes")
    func themeRegistryModes() {
        #expect(DevysThemeRegistry.modes == [.light, .dark, .system])
        #expect(DevysThemeMode.light.preferredColorScheme == .light)
        #expect(DevysThemeMode.dark.preferredColorScheme == .dark)
        #expect(DevysThemeMode.system.preferredColorScheme == nil)

        let systemLight = DevysThemeRegistry.theme(for: .system, systemColorScheme: .light)
        let systemDark = DevysThemeRegistry.theme(for: .system, systemColorScheme: .dark)
        let forcedLight = DevysThemeRegistry.theme(for: .light, systemColorScheme: .dark)
        let forcedDark = DevysThemeRegistry.theme(for: .dark, systemColorScheme: .light)

        #expect(systemLight.isDark == false)
        #expect(systemDark.isDark == true)
        #expect(forcedLight.isDark == false)
        #expect(forcedDark.isDark == true)
    }
