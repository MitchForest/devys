...
public struct EditorMetrics: Equatable, Sendable {
    ...
    public static func measure(
        fontSize: CGFloat,
        fontName: String = "Menlo-Regular",
        lineHeight requestedLineHeight: CGFloat? = nil
    ) -> EditorMetrics {
        let font = RequiredEditorFont.make(name: fontName, size: fontSize)
        ...
        let measuredLineHeight = ceil(ascent + descent + leading).clamped(to: 10...100)
        let lineHeight = max(measuredLineHeight, requestedLineHeight ?? measuredLineHeight)
