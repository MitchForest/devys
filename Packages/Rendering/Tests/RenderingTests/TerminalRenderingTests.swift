...
struct TerminalRenderingTests {
    ...
    @Test("Terminal font stack does not treat LastResort as a usable glyph font")
    func fontStackRejectsLastResortForPrivateUseGlyphs() {
        let stack = TerminalFontStack(primaryFontName: "Menlo", fontSize: 13)
        let resolved = stack.resolve(grapheme: "\u{E73C}")

        #expect(resolved.usesLastResort == false)
        #expect(resolved.isMissing == false)
        #expect(resolved.fontIdentifier == "SymbolsNFM")
    }

    @Test("Terminal font stack resolves eza JS and TS icons to the bundled Nerd Font")
    func fontStackResolvesEzaLanguageIcons() {
        let stack = TerminalFontStack(primaryFontName: "Menlo", fontSize: 13)
        let javascript = stack.resolve(grapheme: "\u{E74E}")
        let typescript = stack.resolve(grapheme: "\u{E628}")

        #expect(javascript.usesLastResort == false)
        #expect(typescript.usesLastResort == false)
        #expect(javascript.fontIdentifier == "SymbolsNFM")
        #expect(typescript.fontIdentifier == "SymbolsNFM")
    }

    @Test("Terminal glyph atlas rasterizes common private-use icons without missing boxes")
    @MainActor
    func glyphAtlasRendersPrivateUseIconsWithoutLastResort() {
        guard let device = MTLCreateSystemDefaultDevice() else { return }
        ...
        atlas.prepareGlyphs(
            for: [
                TerminalGlyphRequest(grapheme: "\u{F07B}", cellSpan: 1),
                TerminalGlyphRequest(grapheme: "\u{E73C}", cellSpan: 1),
                TerminalGlyphRequest(grapheme: "\u{F1C5}", cellSpan: 1),
            ]
        )

        let folder = atlas.entry(for: "\u{F07B}", cellSpan: 1)
        let javascript = atlas.entry(for: "\u{E73C}", cellSpan: 1)
        let image = atlas.entry(for: "\u{F1C5}", cellSpan: 1)

        #expect(folder != .empty)
        #expect(javascript != .empty)
        #expect(image != .empty)
        #expect(atlas.partialTextureUploadCount == 1)
    }

    @Test("Terminal glyph atlas draws distinct bundled Nerd Font language icons")
    @MainActor
    func glyphAtlasDrawsDistinctBundledLanguageIcons() throws {
        guard let device = MTLCreateSystemDefaultDevice() else { return }
        ...
        let javascript = try #require(atlas.renderedBitmapForTesting(grapheme: "\u{E74E}", cellSpan: 1))
        let typescript = try #require(atlas.renderedBitmapForTesting(grapheme: "\u{E628}", cellSpan: 1))

        #expect(javascript != typescript)
        #expect(nonTransparentPixelCount(in: javascript) > 0)
        #expect(nonTransparentPixelCount(in: typescript) > 0)
    }

    @Test("Terminal glyph atlas keeps ASCII glyphs on one baseline")
    @MainActor
    func glyphAtlasKeepsASCIIGlyphsOnOneBaseline() throws {
        guard let device = MTLCreateSystemDefaultDevice() else { return }
        ...
        let glyphs = try ["m", "i", "t", "h"].map { grapheme in
            let bitmap = try #require(atlas.renderedBitmapForTesting(grapheme: grapheme, cellSpan: 1))
            return try #require(alphaRowBounds(in: bitmap, width: 16))
        }
        let lowerEdges = glyphs.map(\.maxY)

        #expect((lowerEdges.max() ?? 0) - (lowerEdges.min() ?? 0) <= 1)
    }
...
private func nonTransparentPixelCount(in rgba: [UInt8]) -> Int {
    stride(from: 3, to: rgba.count, by: 4).reduce(0) { count, index in
        count + (rgba[index] > 0 ? 1 : 0)
    }
}

private func alphaRowBounds(in rgba: [UInt8], width: Int) -> (minY: Int, maxY: Int)? {
    let pixelCount = rgba.count / 4
    guard width > 0, pixelCount >= width else { return nil }
    ...
    for y in 0..<height {
        for x in 0..<width {
            ...
            guard rgba[alphaIndex] > 8 else { continue }
            minY = min(minY, y)
            maxY = max(maxY, y)
        }
    }
