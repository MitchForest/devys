...
/// }
/// ```
public struct DevysSplitView<Content: View, EmptyContent: View>: View {
    @Bindable private var controller: DevysSplitController
    ...
    /// - Parameters:
    ///   - controller: The DevysSplitController managing the tab state
    ...
    public var body: some View {
        let content: (TabItem, PaneID) -> Content = { tabItem, paneId in
            contentBuilder(Tab(from: tabItem), PaneID(id: paneId.id))
        }
        let emptyPane: (PaneID) -> EmptyContent = { internalPaneId in
            emptyPaneBuilder(PaneID(id: internalPaneId.id))
        }
        return SplitViewContainer(
            contentBuilder: content,
            emptyPaneBuilder: emptyPane,
            showSplitButtons: controller.configuration.allowSplits
        ...
        .environment(\.splitColors, controller.splitColors)
    }
}

// MARK: - Convenience initializer with default empty view

public extension DevysSplitView where EmptyContent == DefaultEmptyPaneView {
