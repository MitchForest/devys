...
public final class MetalEditorView: NSView, MTKViewDelegate {
    ...
    var appliedThemeDescriptor: RuntimeThemeDescriptor = ThemeRegistry.descriptor(
        name: CodeViewDesign.dark.syntaxThemeName
    )
    ...
    var usesGlassBackground: Bool = false {
        didSet {
            guard oldValue != usesGlassBackground else { return }
    ...
    var lineNumberColor: SIMD4<Float> = .zero

    /// Default editor text color (linear sRGB)
    var textColor: SIMD4<Float> = .zero
    var diffInsertedLineColor: SIMD4<Float> = .zero
    var diffRemovedLineColor: SIMD4<Float> = .zero
    var diffHeaderLineColor: SIMD4<Float> = .zero
    var diffInsertedTextColor: SIMD4<Float> = .zero
    var diffRemovedTextColor: SIMD4<Float> = .zero
    var diffHeaderTextColor: SIMD4<Float> = .zero
    ...
    /// Cursor color (linear sRGB)
    var cursorColor: SIMD4<Float> = .zero
    
    /// Selection color (linear sRGB)
    var selectionColor: SIMD4<Float> = .zero
    var searchMatchColor: SIMD4<Float> = .zero
    var activeSearchMatchColor: SIMD4<Float> = .zero
    ...
    private func commonInit() {
        ...
        // Create MTKView
        mtkView = GlassCapableMTKView(frame: bounds, device: device)
...
extension MetalEditorView {
    func applySurfaceCompositing() {
        MetalSurfaceCompositing.apply(
            hostView: self,
            metalView: mtkView,
            drawsOpaqueBackground: !usesGlassBackground,
