...
extension MetalEditorView {
        ...
        for cell in packet.cells {
            cellBuffer.addCell(
                EditorCellGPU(
                    ...
                    backgroundColor: displayBackgroundColor(for: cell.backgroundColor),
    ...
    private func displayBackgroundColor(for color: SIMD4<Float>) -> SIMD4<Float> {
        guard usesGlassBackground else { return color }
        if color.matchesRGB(backgroundColor) {
