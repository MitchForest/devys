...
struct MetalEditorViewTests {
    ...
    @Test("Glass-backed editor renders default cells transparent")
    func glassBackedEditorRendersDefaultCellsTransparent() {
        let view = MetalEditorView(frame: CGRect(x: 0, y: 0, width: 800, height: 400))
        view.usesGlassBackground = true
        let document = EditorDocument(
            content: """
            let value = 1
            let other = value + 1
        ...
        #expect(contentCell != nil)
        if let contentCell {
            #expect(contentCell.backgroundColor == SIMD4<Float>(0, 0, 0, 0))
        }
    }
    ...
    func themeSwitchPreservesActiveHighlightsUntilReplacementReady() async throws {
        ...
        #expect(view.appliedThemeDescriptor == ThemeRegistry.descriptor(name: "devys-light"))
    }
    ...
    @Test("Diff documents render header, removed, and inserted line colors")
    func diffDocumentsRenderLineColors() async {
        let view = MetalEditorView(frame: CGRect(x: 0, y: 0, width: 800, height: 400))
        let document = EditorDocument(
            content: """
        ...
        document.configureDiffPresentation(syntaxLanguage: "swift")
        ...
        let contentCells = renderedCells(from: view.cellBuffer)
            .filter { $0.flags & EditorCellFlags.lineNumber.rawValue == 0 }

        #expect(contentCells.contains { $0.backgroundColor == view.diffHeaderLineColor })
        #expect(contentCells.contains { $0.backgroundColor == view.diffRemovedLineColor })
        #expect(contentCells.contains { $0.backgroundColor == view.diffInsertedLineColor })
        #expect(contentCells.contains { cell in
            cell.backgroundColor == view.diffInsertedLineColor &&
                cell.foregroundColor != view.diffInsertedTextColor
        })
    }

    @Test("Glass-backed diff documents keep translucent line backgrounds")
    func glassBackedDiffDocumentsKeepTranslucentLineBackgrounds() async {
        let view = MetalEditorView(frame: CGRect(x: 0, y: 0, width: 800, height: 400))
        view.usesGlassBackground = true
        let document = EditorDocument(
            content: """
        ...
        #expect(contentCells.contains { $0.backgroundColor == view.diffRemovedLineColor })
        #expect(contentCells.contains { $0.backgroundColor == view.diffInsertedLineColor })
        #expect(view.diffHeaderLineColor.w == 0.45)
        #expect(view.diffRemovedLineColor.w == 0.45)
        #expect(view.diffInsertedLineColor.w == 0.45)
        #expect(contentCells.contains { $0.backgroundColor == SIMD4<Float>(0, 0, 0, 0) })
    }
