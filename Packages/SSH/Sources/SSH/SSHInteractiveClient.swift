...
public enum SSHInteractiveClientEvent: Sendable {
    case output(Data)
    case stderr(Data)
    case exitStatus(Int)
    case disconnected
...
public actor SSHInteractiveClient {
    public typealias HostKeyValidator = SSHHostKeyValidator
    public typealias EventSink = @Sendable (SSHInteractiveClientEvent) -> Void
            ...
            case .success(let sshHandler):
                sshHandler.createChannel(childChannelPromise, channelType: .session) { channel, channelType in
                    ...
                    let shellHandler = SSHInteractiveShellHandler(
                        ...
                        initialRows: rows,
                        term: term,
                        command: command?.trimmingCharacters(in: .whitespacesAndNewlines).nilIfEmpty,
...
final class SSHUserAuthenticationDelegate: NIOSSHClientUserAuthenticationDelegate, @unchecked Sendable {
...
final class SSHServerAuthenticationDelegate: NIOSSHClientServerAuthenticationDelegate, @unchecked Sendable {
    typealias HostKeyValidator = SSHHostKeyValidator
        ...
        context
            ...
            .flatMap {
                ...
                if let command = self.command {
                    ...
                    return context.triggerUserOutboundEvent(request)
                }

                return context.triggerUserOutboundEvent(
