...
final class GhosttyMetalTerminalRenderer: NSObject, MTKViewDelegate {
    ...
    var fontSize: CGFloat { fontMetrics.fontSize }
    ...
    init(
        ...
        let fontMetrics = GhosttyTerminalFontMetrics.default(fontSize: fontSize)
    ...
    func draw(in view: MTKView) {
        ...
        // Re-assert layer transparency every frame. MTKView's CAMetalLayer
        // can have `isOpaque` flipped back to true during window attach,
        // drawable resize, or backing-property changes; doing it here makes
        // the translucent path resilient to those resets.
...
private extension GhosttyMetalTerminalRenderer {
    func configureClearColor(for view: MTKView) {
        let opacity = appearance.backgroundOpacity
        if opacity < 1 {
            // Transparent clear lets the window vibrancy show through padding areas
            // (above/below/around the cell grid). Cells paint themselves with their
            // own per-cell alpha on top.
    ...
    private func invalidateRowCache() {
        ...
        rowRangesByIndex.removeAll()
        flattenedCellsCache.removeAll(keepingCapacity: true)
        lastGridSignature = nil
        frameProjection.dirtyState = GhosttyTerminalDirtyState(kind: .full)
    }
    ...
    func cellDescriptors(
        ...
        let defaultBackgroundPacked = appearance.background.packedRGB
        ...
        for cell in row.cells where cell.isRenderable {
            ...
            // Translucent terminals: only cells using the default background color
            // bleed the alpha through. Cells with explicit ANSI/selection/cursor bg
            // stay fully opaque so colored highlights remain crisp.
            let bgAlpha: Float = packedColors.backgroundPacked == defaultBackgroundPacked
            ...
            descriptors.append(
                TerminalCellDescriptor(
                    ...
                    backgroundColor: linearColor(fromPacked: packedColors.backgroundPacked, alpha: bgAlpha),
                    glyph: entry,
