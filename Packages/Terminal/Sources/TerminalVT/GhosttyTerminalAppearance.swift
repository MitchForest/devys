...
public struct GhosttyTerminalAppearance: Sendable, Equatable {
    ...
    /// Alpha applied to cells that use the default background color, so the window
    /// vibrancy (or whatever is behind the Metal layer) shows through. Cells with
    /// explicit ANSI background colors stay fully opaque. 1.0 = fully opaque terminal.
