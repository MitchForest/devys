...
private let supportedThemeDescriptors: [RuntimeThemeDescriptor] = CodeSyntaxThemeDesign.supportedThemes
    .map { RuntimeThemeDescriptor(name: $0.name, version: 1) }
...
public final class ThemeRegistry: ThemeService {
    ...
    public static var preferredThemeName: String {
        if let saved = UserDefaults.standard.string(forKey: "Syntax.themeName") {
            guard supportedThemeNames.contains(saved) else {
                preconditionFailure("Unsupported saved syntax theme '\(saved)'")
            }
            return saved
        }
    ...
    public static func descriptor(name: String) -> RuntimeThemeDescriptor {
        guard let descriptor = supportedThemeDescriptorsByName[name] else {
            preconditionFailure("Unsupported runtime theme '\(name)'")
        }
        return descriptor
    }
    ...
    public var currentThemeName: String = defaultThemeName {
        didSet {
            ...
            guard supportedThemeNames.contains(currentThemeName) else {
                setCurrentThemeName(oldValue)
                return
            }
            applyThemeSelection(
    ...
    private static func canonicalThemeName(for requestedName: String) -> String {
        guard let descriptor = supportedThemeDescriptorsByName[requestedName] else {
            preconditionFailure("Unsupported runtime theme '\(requestedName)'")
        }
    ...
    private func applyThemeSelection(
        ...
        preconditionFailure("Missing bundled runtime theme '\(canonicalName)'")
    }
